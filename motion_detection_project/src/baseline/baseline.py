...
def run_split(
    split_name: str, videos: list[str], cfg, annotations, model: Threshold1D | None
):
    rows = []
    all_scores = []
    all_y = []
...
def main(config_path: str):
    ...
    model, train_metrics, train_df = run_split(
        "train", train_v, cfg, annotations, model=None
    )
    ...
    save_json(
        {"model_type": cfg["model"]["type"], "threshold": float(model.threshold)},
        os.path.join(out_dir, "model.json"),
    )
    save_json(
